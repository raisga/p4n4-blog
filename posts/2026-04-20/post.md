...
date: 2026-04-20
excerpt: Loose components on a desk become a proper integrated unit. A wooden base gets cut, the Pi, hard drive, USB hub, cameras, power, and GPIO boards all mount to it, and the perfboard gets its first solder work.
---

Scripts running, OS sorted, prototype confirmed. The next problem was physical: everything was still just components sitting on a desk, connected by cables that could shift or disconnect at any moment. This session was about fixing that. Designing what gets soldered onto the dev board, and building the wooden base that holds the whole platform together.

## Brainstorming the Protoboard

Before touching the soldering iron, time was spent on what actually belongs on the protoboard. The breadboard circuit works, but breadboards are not permanent. Every connection is one bump away from a false contact, and the platform is supposed to run unattended.

The goal for the protoboard was a clean, solderable version of the LED indicator circuit: the three status LEDs, resistors, and header pins to mate with the GPIO breakout. The KiCad schematic already existed in `core/hw/prototypes/leds-indicator/` from an earlier design pass. This session was about translating that into physical solder joints on a perfboard.
...
- **USB hub** at the rear, providing expansion ports for cameras and other peripherals
- **Cameras** positioned at the front edge, angled for a clean field of view
- **Power outlet board** at the back, feeding the Pi and hub from a single cable in
...
The indicator circuit is straightforward: three LEDs on GPIO17, GPIO27, and GPIO22, each with a 330-ohm resistor to ground. The header pin row lines up with the T-type GPIO breakout so the board seats directly without flying leads. Soldering on perfboard takes longer than placing on a breadboard but the result is mechanically solid. No connections shift under vibration, no pins pull out from handling.
...
The physical platform is done. The dev board is a real thing now, not a pile of parts. The next sessions turn back to software: the remaining GPIO scripts (health monitor, button handler, MQTT indicator) and then the GenAI stack coming up for the first time. Ollama, Letta, and n8n joining the network, and the AI layer of the architecture going from a diagram to a running service.
